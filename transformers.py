...
def prepocess_input_sequence(
    ...
    ##############################################################################
    # Replace "pass" statement with your code
        # split input string into tokens by space
    tokens = input_str.split()

    for tok in tokens:
        # case 1: special token (BOS, EOS, POSITIVE, NEGATIVE, add, subtract)
        if tok in spc_tokens:
            out.append(token_dict[tok])

        # case 2: numeric token (e.g. "0333")
        elif tok.isdigit():
            for digit in tok:          # keep leading zeros!
...
def scaled_dot_product_two_loop_single(
    ...
    # Replace "pass" statement with your code
    K, M = query.shape
    out = torch.empty((K, M), device=query.device, dtype=query.dtype)

    scale = 1.0 / math.sqrt(M)

    for i in range(K):  # loop over queries
        scores = torch.empty((K,), device=query.device, dtype=query.dtype)
        for j in range(K):  # loop over keys
            # scalar dot product between q_i and k_j
            scores[j] = torch.dot(query[i], key[j]) * scale

        # softmax over the K keys for this query
        weights = torch.softmax(scores, dim=0)  # (K,)

        # weighted sum of values: (K,) @ (K, M) -> (M,)
        out[i] = weights @ value
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
...
def scaled_dot_product_two_loop_batch(
    ...
    # Output placeholder (same device/dtype as query)
    
    out = query.new_empty(N, K, M)

    scale = 1.0 / math.sqrt(M)

    for n in range(N):
        # key[n]: (K, M), value[n]: (K, M)
        for t in range(K):
            # q: (M,)
            q = query[n, t]  # (M,)

            # scores: (K,) = (K, M) @ (M,)
            scores = (key[n] @ q) * scale  # (K,)

            # weights: (K,) softmax over keys
            weights = torch.softmax(scores, dim=0)  # (K,)

            # weighted sum: (M,) = (K,) @ (K, M)
            out[n, t] = weights @ value[n]  # (M,)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
def scaled_dot_product_no_loop_batch(
    ...
    weights = torch.matmul(query,(key.transpose(1,2))) / math.sqrt(M)
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        # )
        self.weights_softmax = None  # weight matrix after applying self_attention_no_loop_batch
        y = None
        ##########################################################################
...
class MultiHeadAttention(nn.Module):
    def __init__(self, num_heads: int, dim_in: int, dim_out: int):
        ...
        # 1) ModuleList of single-head attention blocks
        #    Each head: input dim_in, produces output dim_out (value dim)
        ...
        # 2) Linear projection back to dim_in after concatenation
        #    Input: num_heads * dim_out  -> Output: dim_in
        
        self.proj = nn.Linear(num_heads * dim_out, dim_in, bias=True)

        # Weight init like SelfAttention: Uniform[-c, c], c = sqrt(6/(Din+Dout))
        c = math.sqrt(6.0 / (self.proj.in_features + self.proj.out_features))
    ...
    def forward(
        ...
        # Run each head on the same (query,key,value,mask), collect outputs:
        # list of (N, K, dim_out)
        
        head_outs = [head(query, key, value, mask) for head in self.heads]

        # Concatenate along embedding dimension: (N, K, num_heads * dim_out)
        concat = torch.cat(head_outs, dim=2)

        # Project back to (N, K, dim_in)
        y = self.proj(concat)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
class LayerNormalization(nn.Module):
    def __init__(self, emb_dim: int, epsilon: float = 1e-10):
        ...
        # scale (gamma) and shift (beta), shape: (emb_dim,)
    ...
    def forward(self, x: Tensor):
        ...
        # Works for x shaped (N, K) or (N, K, M), etc.
        mean = x.mean(dim=-1, keepdim=True)  # (..., 1)
        var = ((x - mean) ** 2).mean(dim=-1, keepdim=True)  # (..., 1)
        x_hat = (x - mean) / torch.sqrt(var + self.epsilon)  # same shape as x

        # Broadcast gamma/beta over all leading dims
...
class FeedForwardBlock(nn.Module):
    def __init__(self, inp_dim: int, hidden_dim_feedforward: int):
        ...
        # First MLP: (M -> hidden)
        self.fc1 = nn.Linear(inp_dim, hidden_dim_feedforward, bias=True)
        ...
        # Second MLP: (hidden -> M)  (output must return to inp_dim to keep shape)
        self.fc2 = nn.Linear(hidden_dim_feedforward, inp_dim, bias=True)
...
class EncoderBlock(nn.Module):
    def __init__(
       ...
       inp - multi_head_attention - out1 - layer_norm(out1 + inp) - dropout - out2 \ 
        ...
        self.MultiHeadBlock = MultiHeadAttention(num_heads=num_heads, dim_in=emb_dim, dim_out=dim_head)
    ...
    def forward(self, x):
        ...
        # 1) Multi-Head Self-Attention (query=key=value=x)
...
def get_subsequent_mask(seq):
    ...
    # Upper triangular (excluding diagonal): True above diagonal => j > t
    base_mask = torch.triu(
        torch.ones((K, K), device=seq.device, dtype=torch.bool),
        diagonal=1
    )  # (K, K)


    # Expand to batch: (N, K, K)
    mask = base_mask.unsqueeze(0).expand(N, K, K)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return mask
...
class DecoderBlock(nn.Module):
    def __init__(
        ...
        # ---- (1) MultiHeadAttention blocks ----
        # Each head outputs dim_head, concat gives emb_dim, then proj maps back to emb_dim.
        dim_head = emb_dim // num_heads
        self.attention_self = MultiHeadAttention(
            num_heads=num_heads, dim_in=emb_dim, dim_out=dim_head
        )
    ...
    def forward(
        ...
        # # If mask wasn't provided, create subsequent mask for decoder self-attention
        # if mask is None:
        #     # get_subsequent_mask expects (N, K) input (token-like),
        #     # but it only needs the shape, not the values.
        #     dummy_seq = dec_inp.new_zeros((dec_inp.shape[0], dec_inp.shape[1]))
        #     mask = get_subsequent_mask(dummy_seq)  # (N, K_dec, K_dec)

        # 1) Masked self-attention over decoder input
        out1 = self.attention_self(dec_inp, dec_inp, dec_inp, mask)  # (N,K_dec,M)
        out1 = self.norm1(out1 + dec_inp)
        out2 = self.dropout(out1)

        # 2) Cross-attention: query from decoder, key/value from encoder output
        out3 = self.attention_cross(out2, enc_inp, enc_inp)     # (N,K_dec,M)
        out3 = self.norm2(out3 + out2)
        out4 = self.dropout(out3)

        # 3) FeedForward
        out5 = self.feed_forward(out4)   
        out5 = self.norm3(out5 + out4)                             # (N,K_dec,M)
        y = self.dropout(out5)
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    # Reshape to (1, K, 1)
    positions = positions.view(1, K, 1)

    # Repeat along embedding dimension -> (1, K, M)
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    # i here is the *pair index* (0,1,2,...) not the embedding-dim index
    num_pairs = (M + 1) // 2
    i = torch.arange(num_pairs, dtype=torch.float32)  # (num_pairs,)

    # a = floor(2i / M)
    a = torch.floor((2 * i) / M)  # (num_pairs,)

    # divisor = 10000^a
    div = torch.pow(torch.tensor(10000.0), a)  # (num_pairs,)

    pe[:, 0::2] = torch.sin(p / div)  # even dims
    pe[:, 1::2] = torch.cos(p / div[: pe[:, 1::2].shape[1]])  # odd dims (safe for odd M)
...
class Transformer(nn.Module):
    ...
    def forward(
        ...
        enc_out = self.encoder(q_emb_inp)               # (N, Kq, M)
        mask = get_subsequent_mask(ans_b[:, :-1])       # (N, Ka-1, Ka-1)
        dec_out = self.decoder(a_emb_inp, enc_out, mask)  # (N, Ka-1, V)
        N, O, V = dec_out.shape
